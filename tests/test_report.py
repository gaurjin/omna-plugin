...
def test_report_aggregates_receipts():
    receipts.append({"route": "/v1/messages", "upstream": "api.anthropic.com", "status": 200, "stream": True, "masked": {"EMAIL": 2, "AWS_KEY": 1}, "secrets": 1, "pii": 2, "ms": 900, "session": "s1"})
    receipts.append({"route": "/v1/messages", "upstream": "api.anthropic.com", "status": 200, "stream": True, "masked": {}, "secrets": 0, "pii": 0, "ms": 300, "session": "s1"})
    ...
    assert "3 AI requests enabled" in text and "1 secrets kept off the wire" in text and "INTACT" in text
